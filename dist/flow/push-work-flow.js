    ...
    try {
        const answers = await inquirer.prompt([
            ...
            },
            {
                type: 'input',
                name: 'githubRepoUrl',
                message: 'Enter your skill github repo url:',
            },
        ]);
        handlePushSkill({ repoUrl: answers.githubRepoUrl, skillName: answers.skillName });
    }
    catch (error) {
        console.error(error);
    }
...
//   }
// };
export const handlePushSkill = async ({ repoUrl, skillName }) => {
    try {
        const apiKey = await getApiKey();
        const { desc, mainContent } = await getSkillInfo(skillName);
...
//# sourceMappingURL=push-work-flow.js.map